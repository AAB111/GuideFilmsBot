...
@pytest.mark.asyncio
async def test_search_not_valid_user():
    message = AsyncMock()
...
@pytest.mark.asyncio
async def test_search_movies_len_0():
    movies = []
    message = AsyncMock()
    message.from_user.id = TEST_USER.id
    message.text = "test_overview"
    with patch('handlers.search.get_search_movies', return_value=movies) as mock_get_search_movies:
        await start_search(message)
        assert mock_get_search_movies.call_count == 1
        assert mock_get_search_movies.call_args == call(message.text, message.from_user.id)
        message.reply.assert_awaited_with("К сожалению, ничего не найдено. Попробуйте составить запрос позже.")
...
async def test_search_text():
    ...
    with mock.patch('handlers.search.get_search_movies', return_value=movies) as mock_get_search_movies:
        message = AsyncMock()
        message.from_user.id = TEST_USER.id
        await start_search(message)

        assert mock_get_search_movies.call_count == 1
        assert mock_get_search_movies.call_args == mock.call(message.text, message.from_user.id)
...
@pytest.mark.asyncio
async def test_search_media_all():
    movies = [
        {
            "id": 447365,
            "title": "Стражи Галактики Том. 3",
